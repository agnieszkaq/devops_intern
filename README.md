...
5. To check if string is palindrome, write in browser adress: 
  ```
  http://localhost:8080/isPalindrome?text=yourSpecyficWord
  
  ```
5. At browser you will see information if that specyfic word is palindrome (information below "true") or no (information below "false").

### Stage 2
*Download Jenkins and run as a docker image on your local machine. Write
step by step how to do it.*

Before we download Jenkins we need to install docker on machine. It's neded to create image with Jenkins. 
There are steps for Linux Mint (it's my system):
1. On console please write:
  ...
  ```
2. Next step is to create Jenkins container in Docker, with special privileges. We use this privileges in **Stage 5** build Docker Image inside Jenkins, wtihout this You will get errors ("docker not found"), like me before, when I did'n know about this:
  ```
  ...
  ```
  Save the password process installtion. Next, go to browser on adress: **localhost:8080**
  On this adress, click on *Install suggested plugins*. Wait to installation process would be done (it takes some time).
  After that, fill the inputs and create administrator. 
...
### Stage 4
*Create a Jenkins Pipeline that builds your spring boot application when changes are pushed to Github.*
...
In ***Repository URL***, put https://github.com/agnieszkaq/devops.git. Don't forget to put branch (In mine case it will be *main*).
After that, check that script path is the same like default. It should be ***Jenkinsfile***.

To push GitHub changes we will use Webhooks. My Jenkins server is on localhost adress, the problem is that Jenkins server is sitting behind a firewall and/or NAT, so the GitHub webhook cannot be delivered to your localhost Jeninks server. 
...
2. In GitHub:
    - go to ***Project Settings*** -> ***Webhooks*** -> fill ***Payload URL***, with your Jenkins adress which socketxp generate( like this: *socketxp_adress>/github-webhook/*).

3. Then, you can check if is working, by pushing your git command.
...
```
3. Inside directory with project ***:/var/jenkins_home/workspace/devops_intern***, wrtie build commands in console:
```
...
Firstly, create Your Docker Hub account. Then, we need to generate access token. To do this follow the steps:
click on your ***username*** on the top right, next ***Accoutn Setting** ->  **Security** -> **New access token***. 
Please, save the token. This token it will be added to ***Manage Credentials*** in Jenkins, to create connection beetween Jenkins and Docker Hub. 
Add this token the same like u did this with GitHub token. 
...
```
After git commit, Spring Boot application is added to Docker Hub!
...
Now, in browser You can go to: *http://localhost:8081/isPalindrome?text=yourSpecyficWord*, to check if your specyfic word is palindrome.
